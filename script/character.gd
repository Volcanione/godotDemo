...
# 射击方向优先与移动方向，用于决定当前显示的角色朝向
# 自动螺旋弹幕期间不在读取射击输入，而是仅按移动方向更新 armed 动画朝向。
...
# 道具统一通过整个入口影响玩家，Pickup 场景不直接改玩家内部细节
func apply_pickup(config:PickupConfig) ->bool:
	...
	if not is_equal_approx(config.move_speed_mulitplier,DEFAULT_MOVE_SPEED_MULTIPLIER):
		current_move_speed_multiplier = config.move_speed_mulitplier
...
#根据当前弹幕模式发射子弹，并返回这次是否至少成生成了一枚子弹
...
func _update_invincibilty(delta:float) -> void:
	if invincibility_time_left <= 0.0:
		return
...
# 统一刷新射击计时器的基础问题，避免 buff 生效后仍使用就数值
func _refresh_shooting_timer_wait_time() -> void:
	...
	#如果玩家在冷却途中拾取了更快的射速buff ，需要让当前这次冷却也立即缩短
...
# 强化螺旋形态下显示特殊动画，结束后隐藏并停止播放

func _update_armed_effect() -> void:
	var is_armed := current_form_mode == PickupConfig.PlayerFormMode.ARMED

	if not is_armed:
		if armed_ecefft_sprite.visible:
			armed_ecefft_sprite.visible = false
		if armed_ecefft_sprite.is_playing():
			armed_ecefft_sprite.stop()
		return

	if not armed_ecefft_sprite.visible:
		armed_ecefft_sprite.visible = true
	if armed_ecefft_sprite.is_playing():
		return
	if armed_ecefft_sprite.sprite_frames == null:
		return
