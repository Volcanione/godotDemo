...
func _ready() -> void:
	...
	animated_sprite.animation_finished.connect(_on_animated_sprite_animaiton_finished)
	_apply_config()
...
# 管理与玩家持续接触时的伤害冷却
func _update_touch_damage(delat:float) -> void:
	if touch_damage_cooldown_left > 0.0:
		touch_damage_cooldown_left = maxf(touch_damage_cooldown_left - delat, 0.0)
...
#统一设置手机闪烁开关，避免掉落重复的材质访问代码
func _set_hurt_blink_enabled(enable:bool) -> void:
	var sprite_material := animated_sprite.material as ShaderMaterial
	if sprite_material != null:
		sprite_material.set_shader_parameter(BLINK_ENABLED_SHADER_PARAMETER,enable)
...
func _start_death_sequence() -> void:
	...
	Tools.play_sfx(die_sfx_player)
		
	if _play_death_sequence_animation(config.death_animation_name,DeathSequenceStage.DEATH):
		return
		
	_finish_fater_death_animation()
	

#普通敌人在死亡动画结束后直接销毁，自爆敌人则进入第二阶段爆炸流程
func _finish_fater_death_animation() -> void:
...
func _start_explosion_sequence() -> void:
	...
	Tools.play_sfx(explode_sfx_player)
...
#统一切换死亡阶段动画，找不到动画 时返回false, 由上层决定如何处理
...
# 延迟到当前物理拆线呢结束后再实例化凋落物，避免再碰撞回调中直接修改物理对象状态。
...
func _on_animated_sprite_animaiton_finished() -> void:
	if not is_dead:
		return
