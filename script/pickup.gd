...
const BLINK_ENABLED_SHADER_PARAMTER := &"blink_enabled"
...
@onready var lifttime_timer: Timer = $LifttimeTimer
...
func  _ready() -> void:
	...
	lifttime_timer.timeout.connect(_on_lifetime_timer_timeout)
	lifttime_timer.one_shot = true
	if lifttime_timer.wait_time > 0.0:
		lifttime_timer.start()
...
func _process(delta: float) -> void:
	...
	if lifttime_timer.is_stopped():
		return
	if lifttime_timer.time_left > blink_before_expire:
		return
...
func _set_blink_enabled(enabled:bool) -> void:
	...
	if sprite_material != null:
		sprite_material.set_shader_parameter(BLINK_ENABLED_SHADER_PARAMTER,enabled)
